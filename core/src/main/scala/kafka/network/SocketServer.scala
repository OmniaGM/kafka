...
object DataPlaneAcceptor {
  ...
  val ListenerReconfigurableConfigs: Set[String] = Set(ServerConfig.NUM_NETWORK_THREADS_CONFIG)
}
...
class DataPlaneAcceptor(socketServer: SocketServer,
  ...
  override def reconfigure(configs: util.Map[String, _]): Unit = {
    val newNumNetworkThreads = configs.get(ServerConfig.NUM_NETWORK_THREADS_CONFIG).asInstanceOf[Int]
  ...
  override def configure(configs: util.Map[String, _]): Unit = {
    addProcessors(configs.get(ServerConfig.NUM_NETWORK_THREADS_CONFIG).asInstanceOf[Int])
  }
}
