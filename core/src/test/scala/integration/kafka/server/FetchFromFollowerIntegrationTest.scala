...
class FetchFromFollowerIntegrationTest extends BaseFetchRequestTest {
  ...
  def testRackAwareRangeAssignor(quorum: String): Unit = {
    ...
    val consumers = brokers.map { server =>
      ...
      consumerConfig.setProperty(ConsumerConfig.CLIENT_RACK_CONFIG, server.config.serverConfig.rack.orElse(null))
      consumerConfig.setProperty(ConsumerConfig.GROUP_INSTANCE_ID_CONFIG, s"instance-${server.config.serverConfig.brokerId}")
