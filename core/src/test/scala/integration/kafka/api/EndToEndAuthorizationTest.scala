...
import org.apache.kafka.server.config.{ServerConfig, ReplicationConfigs, ServerLogConfigs, ZkConfigs}
...
abstract class EndToEndAuthorizationTest extends IntegrationTestHarness with SaslSetup {
  ...
  override def setUp(testInfo: TestInfo): Unit = {
    ...
    if (TestInfoUtils.isKRaft(testInfo)) {
      ...
      this.serverConfig.setProperty(ServerConfig.AUTHORIZER_CLASS_NAME_CONFIG, classOf[StandardAuthorizer].getName)
      this.controllerConfig.setProperty(ServerConfig.AUTHORIZER_CLASS_NAME_CONFIG, classOf[StandardAuthorizer].getName)
    } else {
      ...
      this.serverConfig.setProperty(ServerConfig.AUTHORIZER_CLASS_NAME_CONFIG, authorizerClass.getName)
