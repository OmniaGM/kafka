...
class DynamicNumNetworkThreadsTest extends BaseRequestTest {
  ...
  override def brokerPropertyOverrides(properties: Properties): Unit = {
    ...
    properties.put(s"listener.name.${internal.toLowerCase}.${ServerConfig.NUM_NETWORK_THREADS_CONFIG}", "2")
    properties.put(ServerConfig.NUM_NETWORK_THREADS_CONFIG, ServerConfig.NUM_NETWORK_THREADS_DEFAULT.toString)
  }
  ...
  override def setUp(testInfo: TestInfo): Unit = {
    ...
    assertEquals(ServerConfig.NUM_NETWORK_THREADS_DEFAULT, getNumNetworkThreads(external))
  }
  @AfterEach
  ...
  def testDynamicNumNetworkThreads(quorum: String): Unit = {
    ...
    val newBaseNetworkThreadsCount = ServerConfig.NUM_NETWORK_THREADS_DEFAULT + 1
    var props = new Properties
    props.put(ServerConfig.NUM_NETWORK_THREADS_CONFIG, newBaseNetworkThreadsCount.toString)
    reconfigureServers(props, (ServerConfig.NUM_NETWORK_THREADS_CONFIG, newBaseNetworkThreadsCount.toString))
    ...
    props.put(s"listener.name.${internal.toLowerCase}.${ServerConfig.NUM_NETWORK_THREADS_CONFIG}", newInternalNetworkThreadsCount.toString)
    reconfigureServers(props, (s"listener.name.${internal.toLowerCase}.${ServerConfig.NUM_NETWORK_THREADS_CONFIG}", newInternalNetworkThreadsCount.toString))
