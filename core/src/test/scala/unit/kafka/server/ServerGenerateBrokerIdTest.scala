...
class ServerGenerateBrokerIdTest extends QuorumTestHarness {
  ...
  def testAutoGenerateBrokerId(): Unit = {
    ...
    servers = Seq(server1)
    assertEquals(server1.config.serverConfig.brokerId, 1001)
    server1.shutdown()
  ...
  def testUserConfigAndGeneratedBrokerId(): Unit = {
    ...
    assertEquals(server1.config.serverConfig.brokerId, 1001)
    server2.startup()
    assertEquals(server2.config.serverConfig.brokerId, 0)
    server3.startup()
    assertEquals(server3.config.serverConfig.brokerId, 1002)
  ...
  def testBrokerMetadataOnIdCollision(): Unit = {
    ...
    propsB.setProperty(ServerConfig.BROKER_ID_CONFIG, "2")
    ...
    propsB.setProperty(ServerConfig.BROKER_ID_CONFIG, "3")
