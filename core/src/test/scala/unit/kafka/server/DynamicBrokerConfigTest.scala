...
import org.apache.kafka.server.config.{KRaftConfigs, ReplicationConfigs, ServerConfig, ServerLogConfigs, ZkConfigs}
...
class DynamicBrokerConfigTest {
  ...
  def testUpdateDynamicThreadPool(): Unit = {
    val origProps = TestUtils.createBrokerConfig(0, TestUtils.MockZkConnect, port = 8181)
    origProps.put(ServerConfig.NUM_IO_THREADS_CONFIG, "4")
    origProps.put(ServerConfig.NUM_NETWORK_THREADS_CONFIG, "2")
    ...
    origProps.put(ServerConfig.BACKGROUND_THREADS_CONFIG, "3")
    ...
    assertTrue(captor.getValue.containsKey(ServerConfig.NUM_NETWORK_THREADS_CONFIG))
    assertEquals(4, captor.getValue.get(ServerConfig.NUM_NETWORK_THREADS_CONFIG))
    ...
    props.put(ServerConfig.BACKGROUND_THREADS_CONFIG, "6")
    config.dynamicConfig.updateDefaultConfig(props)
  ...
  def testReconfigurableValidation(): Unit = {
    val origProps = TestUtils.createBrokerConfig(0, TestUtils.MockZkConnect, port = 8181)
    val config = KafkaConfig(origProps)
    val invalidReconfigurableProps = Set(CleanerConfig.LOG_CLEANER_THREADS_PROP, ServerConfig.BROKER_ID_CONFIG, "some.prop")
  ...
  def testDynamicConfigInitializationWithoutConfigsInZK(): Unit = {
    ...
    initialProps.remove(ServerConfig.BACKGROUND_THREADS_CONFIG)
    ...
    newprops.put(ServerConfig.NUM_IO_THREADS_CONFIG, "10")
    newprops.put(ServerConfig.BACKGROUND_THREADS_CONFIG, "100")
  ...
  def testImproperConfigsAreRemoved(): Unit = {
    ...
    newProps.put(ServerConfig.MESSAGE_MAX_BYTES_CONFIG, "2222")
    ...
    newProps.put(ServerConfig.MESSAGE_MAX_BYTES_CONFIG, "1111")
...
class TestDynamicThreadPool() extends BrokerReconfigurable {
  ...
  override def reconfigure(oldConfig: KafkaConfig, newConfig: KafkaConfig): Unit = {
    assertEquals(ServerConfig.NUM_IO_THREADS_DEFAULT, oldConfig.serverConfig.numIoThreads)
    assertEquals(ServerConfig.BACKGROUND_THREADS_DEFAULT, oldConfig.serverConfig.backgroundThreads)
