...
import org.apache.kafka.server.config.{ServerConfig, QuotaConfigs}
...
class RequestQuotaTest extends BaseRequestTest {
  ...
  override def brokerPropertyOverrides(properties: Properties): Unit = {
    ...
    properties.put(ServerConfig.UNSTABLE_API_VERSIONS_ENABLE_CONFIG, "true")
    if (isKRaftTest()) {
      properties.put(ServerConfig.AUTHORIZER_CLASS_NAME_CONFIG, classOf[RequestQuotaTest.KraftTestAuthorizer].getName)
    } else {
      properties.put(ServerConfig.AUTHORIZER_CLASS_NAME_CONFIG, classOf[RequestQuotaTest.ZkTestAuthorizer].getName)
    }
  }
